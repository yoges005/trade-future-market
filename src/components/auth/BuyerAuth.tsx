...
import { ArrowLeft, ShoppingBag, Eye, EyeOff, MapPin } from "lucide-react";
...
const BuyerAuth = ({ mode, onBack, onModeChange }: BuyerAuthProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "signup") {
      ...
      toast.success("Account created successfully! Welcome to FutureMarket!");
    } else {
        ...
        <Card className="backdrop-blur-sm bg-white/80 border-white/20 shadow-2xl">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl font-bold text-purple-700">
              {mode === "login" ? "Welcome Back, Buyer!" : "Join as a Buyer"}
            </CardTitle>
            <p className="text-gray-600 text-sm">
              {mode === "login" 
                ? "Sign in to find amazing deals on pre-owned items"
                : "Create your account to start shopping second-hand treasures"
              }
            </p>
          </CardHeader>
